...
class AuthService {
  async loginWithOpenId(payload) {
    try {
      const { id_token } = payload;
      const profile = jwt2.decode(_.trim(id_token));
      const userPayload = {
        openId: profile.id,
      ...
      if (checkUser) {
        let data = await Models.User.query().updateAndFetchById(checkUser.id, userPayload);
        data.scope = "user";
        ...
        return this.register(userPayload);
      }
    } catch (err) {
      throw err;
    }
